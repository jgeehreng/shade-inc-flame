#!/usr/bin/env python3
"""
Flame Launch hook: installs required Python packages for Shade integration.
...
def _get_sudo_password_gui(packages: List[Tuple[str, str]]) -> Optional[str]:
    ...
    try:
        ...
        dialog.setWindowTitle("Shade Package Installer - Administrator Password Required")
        ...
        print(f"[shade_packages] GUI password dialog failed: {e}, falling back to terminal input")
        try:
            return getpass.getpass("Administrator password required to install system-wide packages (sudo): ")
        except (KeyboardInterrupt, EOFError):
            return None
...
def _install_missing_with_sudo(missing: List[Tuple[str, str]]) -> None:
    ...
    print("[shade_packages] Installing required packages…")
...
def install_python_packages(_selection=None):
    try:
        ...
        print("[shade_packages] All required packages are installed.")
    except Exception as e:
        print(f"[shade_packages] {e}")
